...
const agregarAuto=async()=>{
  ...
  if(marca.trim()===''|| modelo.trim()===''|| color.trim()==='' ||
     año.trim()===''|| costo.trim()===''){
      Swal.fire({
          icon: 'error',
          title: 'ERROR',
...
const cargarAuto=async(autoid)=>{
  ...
  datos.append("autoid",autoid);
    ...
    document.querySelector("#colorEdit").value=resultado.color;
    document.querySelector("#añoEdit").value=resultado.año;
...
const editarAuto=async()=>{
    ...
    var color=document.querySelector("#colorEdit").value;
    var año=document.querySelector("#añoEdit").value;
